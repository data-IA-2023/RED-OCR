...
def get_data_from_all_invoice():
    ...
    for invoice in range(len(get_all_invoices_from_lastdate_in_database_because_azure_suck)):
            #print(all_invoice[invoice]['no'])
            data_invoice      = extract_data(get_all_invoices_from_lastdate_in_database_because_azure_suck[invoice]['no'])
            print(data_invoice)
            print('*************************************')
            data_qr           = extract_qr_data(get_all_invoices_from_lastdate_in_database_because_azure_suck[invoice]['no'])
            print(data_qr)
            print('*************************************')
            data_invoice_text = get_brut_format(data_invoice[get_all_invoices_from_lastdate_in_database_because_azure_suck[invoice]['no']])
            get_products_data = get_invoice_products_data(data_invoice_text)
            get_invoice_data  = get_invoices_basic_data(data_invoice_text)
            get_client_data   = get_invoices_client_data(data_qr,data_invoice_text)
